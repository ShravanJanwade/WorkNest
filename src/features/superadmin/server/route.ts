...
import { DATABASE_ID, COMPANIES_ID, WORKSPACES_ID, MEMBERS_ID, B2_BUCKET_NAME } from "@/config";
...
import { createCompanyWithAdminSchema, approveDeleteSchema } from "../schemas";
...
const app = new Hono()
  .get("/check", async (c) => {
    try {
      ...
      const existingUsers = await users.list([Query.equal("email", SUPER_ADMIN_EMAIL)]);
  ...
  .get("/companies", sessionMiddleware, async (c) => {
    ...
    try {
      ...
      const companies = await databases.listDocuments<Company>(DATABASE_ID, COMPANIES_ID, [
  ...
  .post(
    ...
    async (c) => {
      ...
      const { companyName, companyDescription, adminName, adminEmail } = c.req.valid("json");
      ...
      try {
        ...
        const company = await databases.createDocument(DATABASE_ID, COMPANIES_ID, ID.unique(), {
          name: companyName,
        ...
        console.log(
          `ℹ️  Admin should use "Forgot Password" on the login page to set their password`,
        );
  ...
  .post(
    ...
    async (c) => {
      ...
      try {
        ...
        if (approved) {
          await databases.updateDocument(DATABASE_ID, COMPANIES_ID, companyId, {
            status: "deleted",
          });
  ...
  .get("/stats", sessionMiddleware, async (c) => {
    ...
    try {
      ...
      const allCompanies = await databases.listDocuments<Company>(DATABASE_ID, COMPANIES_ID, []);

      const activeCompanies = allCompanies.documents.filter((c) => c.status === "active").length;

      const pendingDeletes = allCompanies.documents.filter((c) => c.deleteRequested).length;
