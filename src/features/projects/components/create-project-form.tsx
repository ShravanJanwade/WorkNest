...
async function urlToFile(url: string, defaultBaseName = "project-ai"): Promise<File> {
  ...
  const ext = (extFromName || extFromMime || "png").replace(/[^a-z0-9]/gi, "").toLowerCase();
...
function ImageGenerator({
  ...
  async function handleUseGenerated() {
    try {
      ...
      setGenError((err as Error)?.message || "Failed to convert generated image");
    }
  }
      ...
      <div className="flex items-center gap-x-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-x-2 mt-3">
            ...
            {value ? (
              <Button size="xs" variant="destructive" onClick={handleRemove} disabled={isPending}>
                Remove
              </Button>
            ) : null}
          </div>
      ...
      {isGenOpen && (
        <div className="mt-2 p-4 rounded-lg border bg-white shadow-sm">
          ...
          <div className="mt-3 flex items-center gap-x-3">
            <Button size="sm" onClick={handleGenerate} disabled={isGenerating} type="button">
...
export const CreateProjectForm = ({ onCancel }: { onCancel?: () => void }) => {
  ...
  const [fieldImageValue, setFieldImageValue] = useState<File | string | null>(null);
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <ImageGenerator
                        value={fieldImageValue ?? (field.value as File | string | null)}
                        setValue={(v) => {
                          if (v instanceof File) {
                            form.setValue("image", v, { shouldTouch: true });
                            setFieldImageValue(v);
