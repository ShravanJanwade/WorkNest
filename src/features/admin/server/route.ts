...
const app = new Hono()
  ...
  .post("/company", sessionMiddleware, zValidator("form", companySchema), async (c) => {
    const databases = c.get("databases");
    const user = c.get("user");
    const { name, description, website, address, image } = c.req.valid("form");

    try {
      const existing = await databases.listDocuments(DATABASE_ID, COMPANIES_ID, [
        Query.equal("adminUserId", user.$id),
      ]);
  ...
  .patch(
    ...
    async (c) => {
      ...
      try {
        const company = await databases.getDocument<Company>(DATABASE_ID, COMPANIES_ID, companyId);
  ...
  .get("/users", sessionMiddleware, async (c) => {
    ...
    try {
      const adminMemberships = await databases.listDocuments(DATABASE_ID, MEMBERS_ID, [
  ...
  .post("/users/invite", sessionMiddleware, zValidator("json", inviteUserSchema), async (c) => {
    const databases = c.get("databases");
    const user = c.get("user");
    ...
    try {
      const inviterMember = await databases.listDocuments(DATABASE_ID, MEMBERS_ID, [
        Query.equal("userId", user.$id),
      ...
      const existingMember = await databases.listDocuments(DATABASE_ID, MEMBERS_ID, [
