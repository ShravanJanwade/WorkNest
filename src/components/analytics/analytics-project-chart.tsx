...
interface AnalyticsProjectChartProps {
...
export const AnalyticsProjectChart = ({ data, title, description }: AnalyticsProjectChartProps) => {
      ...
      <CardContent className="p-2 sm:p-4">
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <defs>
                <linearGradient id="project-gradient" x1="0" y1="0" x2="1" y2="0">
                  <stop offset="0%" stopColor="hsl(340, 75%, 55%)" stopOpacity={1} />
                  <stop offset="100%" stopColor="hsl(15, 85%, 55%)" stopOpacity={0.9} />
                </linearGradient>
              </defs>
              <CartesianGrid 
                strokeDasharray="3 3" 
              ...
              <Tooltip
                ...
                formatter={(value: number) => [`${value} tasks`, "Total"]}
              />
              <Bar 
                dataKey="value" 
                radius={[0, 6, 6, 0]} 
              ...
              >
                {data.map((_, index) => (
                  <Cell 
                    key={`cell-${index}`}
                    fill={`url(#${CHART_GRADIENT_COLORS[(index + 2) % CHART_GRADIENT_COLORS.length].id})`}
                  />
                ))}
              </Bar>
            </BarChart>
